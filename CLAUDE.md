...
| LLM (all chat/reasoning) | OpenAI (`openai` SDK) — `gpt-4.1` (chat), `gpt-4o-mini` (titles). Powers RAG synthesis, review, security audit, refactor proposals, agent loop, file chat, tours. Shared client + `streamChatText` helper in `backend/src/ai/openaiClient.js` |
...
│   │   ├── agentTools.js             # OpenAI function-schema tools + handlers for the agent (US-068)
...
│   ├── ai/                 # openaiClient.js (shared OpenAI client + streamChatText), ragService.js (RAG pipeline)
...
| `*` | `/api/search/*` | RAG code search (OpenAI synthesis + pgvector HNSW) |
...
- `agent_conversations` — AI Repo Agent chats (US-067): `{ id, repo_id, user_id, title, total_tokens, created_at, updated_at }`. `title` is auto-generated by a one-shot `gpt-4o-mini` call after the first assistant turn.
- `agent_messages` — full tool-call trace for the agent (US-067). `role` enum `user | assistant | tool_use | tool_result`; `content_json` is polymorphic by role (`{ text }` for user/assistant, `{ tool_name, input, tool_use_id }` for tool_use, `{ tool_use_id, output, is_error }` for tool_result). The DB shape is provider-neutral; `tool_use_id` stores OpenAI's `tool_call` id (rehydration maps it to `tool_calls[].id` / `tool_call_id`) so traces replay verbatim.
...
OPENAI_API_KEY        # embeddings (text-embedding-3-small) + all chat/reasoning features
OPENAI_CHAT_MODEL     # optional, default gpt-4.1 — review/audit/proposals/agent/RAG/file-chat/tours
OPENAI_TITLE_MODEL    # optional, default gpt-4o-mini — agent conversation titles
NODE_ENV, PORT
...
A conversational surface that drives an OpenAI tool-use loop over CodeLens's deterministic analysis. Replaces the old "Search" tab — the existing RAG retrieval is now one tool (`search_code`) among many. Conversations and the full tool-call trace persist so investigations resume across sessions and are auditable.

**Schema (US-067):** `agent_conversations` + `agent_messages` (see Database Schema above). `content_json` is polymorphic by role; the provider-neutral `tool_use_id` stores OpenAI's `tool_call` id so traces replay verbatim.
...
- `propose_fix({ issue_id })` — only write-side tool. Calls `reviewController._private` (`fetchAnalysisIssue`, `buildContextForIssue`, `streamLLM`, `parseProposalJson`, `normalizeProposal`) non-HTTP and inserts into `issue_proposals`. Returns `{ proposal_id, summary, change_count, risk_count }`.
...
- Persistence-as-you-go: every `assistant` text, `tool_use`, and `tool_result` is INSERTed into `agent_messages` before the next OpenAI call. A dropped connection leaves the conversation resumable.
...
- Streaming (`_private.consumeStream`): accumulates `delta.content` and `delta.tool_calls[]` fragments (by `index`), parsing tool-call arguments once at stream end; usage comes from the final `include_usage` chunk. The loop ends when no tool calls are requested (`finish_reason === 'stop'`).
- Rehydration (`_private.rehydrateMessages`): consecutive `assistant` + `tool_use` rows fold into one OpenAI assistant message carrying `tool_calls`; each `tool_result` becomes its own `role: 'tool'` message (immediately following the assistant message that called it).
- Title generation: fire-and-forget `gpt-4o-mini` (non-streaming) call after the first assistant turn completes; only updates `title` if it is still null.
...
- **`POST /api/review/:id/duplication-refactor`** — SSE-streamed LLM proposal for extracting a shared utility across a cluster.
...
- **Generate** (`POST /:repoId/tours/generate`, `aiRateLimit`) — the LLM writes an ordered list of steps anchored to specific files/lines with prose.
...
`backend/src/controllers/fileChatController.js` — `POST /api/file-chat/:repoId` accepts a file path + a question and streams an LLM response grounded in the file content. Reuses the per-user daily token budget.
...
- **SSE streaming:** AI review and security audit use Server-Sent Events; frontend reads with `ReadableStream` + `TextDecoder`; abort via `AbortController`. Server-side, every LLM stream binds `req.on('close')` to an `AbortController` via `backend/src/lib/sseAbort.js` (`bindRequestAbort` / `isAbortError`) so a closed tab stops generating tokens mid-stream
